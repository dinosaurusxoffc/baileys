...
exports.isWABusinessPlatform =
  ...
  exports.encodeNewsletterMessage =
...
// Unified prefix generator with JKT48 member names
const generateUnifiedPrefix = (type) => {
  const prefixes = {
    message: [74, 75, 84, 52, 56], // JKT48
    messageV2: [65, 68, 69, 76, 73, 78, 69], // ADELINE
    registration: [82, 69, 71, 73, 69], // REGIE
    mdTag: [79, 76, 73, 78, 69], // OLINE
    crockford: [69, 82, 73, 78, 69], // ERINE
    timeout: [76, 73, 76, 89], // LILY
    session: [78, 65, 76, 65], // NALA
    connection: [77, 73, 67, 72, 73, 69], // MICHIE
    generic: [69, 76, 76, 65], // ELLA
  };

  const codes = prefixes[type] || prefixes["generic"];
  return convertToBytes(codes) + "-"; // Added separator
};
exports.generateUnifiedPrefix = generateUnifiedPrefix;

const generateRegistrationId = () => {
  const prefix = generateUnifiedPrefix("registration");
  const id = Uint16Array.from((0, crypto_1.randomBytes)(2))[0] & 16383;
...
const delayCancellable = (ms, includeId = false) => {
  ...
  const cancel = () => {
    ...
    const errorId = includeId
      ? generateUnifiedPrefix("timeout") + Date.now()
      : "";
    reject(
...
const generateMessageIDV2 = (userId) => {
  ...
  const prefix = generateUnifiedPrefix("messageV2");
  return prefix + hash.toString("hex").toUpperCase().substring(0, 11);
};
exports.generateMessageIDV2 = generateMessageIDV2;

const generateMessageID = () => {
  const prefix = generateUnifiedPrefix("message");
  return prefix + (0, crypto_1.randomBytes)(18).toString("hex").toUpperCase();
};
...
const fetchLatestBaileysVersion = async (options = {}) => {
  // Validasi package.json terlebih dahulu
  try {
    const packageJson = require("../../../package.json");
    const requiredPackageName = "@jkt48connect-corp/baileys";

    if (packageJson.name !== requiredPackageName) {
      const error = new Error(
        `kenapa baileys gw dicolong terus jadi ini ${
    ...
    let errorMessage;
    if (packageError.code === "MODULE_NOT_FOUND") {
      errorMessage =
        "package.json not found. Please ensure this is a valid npm package.";
    } else if (
      packageError.message &&
      packageError.message.includes("Invalid package name")
    ) {
      // Jika sudah error validasi nama package, langsung return
      return {
        version: baileys_version_json_1.version,
        isLatest: false,
        error: packageError,
      ...
      errorMessage = `Failed to read package.json: ${packageError.message}`;
    }
  ...
  // Jika validasi package.json berhasil, lanjutkan fetch version
  const URL =
    "https://unpkg.com/@jkt8connect-corp/baileys@7.5.9/lib/Defaults/baileys-version.json";
  try {
...
const generateMdTagPrefix = () => {
  ...
  const prefix = generateUnifiedPrefix("mdTag");
  return prefix + `${bytes.readUInt16BE()}.${bytes.readUInt16BE(2)}-`;
};
...
function bytesToCrockford(buffer, withPrefix = true) {
  let value = 0;
  let bitCount = 0;
  const crockford = [];
  ...
  return withPrefix ? generateUnifiedPrefix("crockford") + result : result;
}

// New function to generate generic IDs with unified prefix
const generateGenericId = (length = 16, type = "generic") => {
  const prefix = generateUnifiedPrefix(type);
  const randomData = (0, crypto_1.randomBytes)(length);
  return prefix + randomData.toString("hex").toUpperCase();
};
exports.generateGenericId = generateGenericId;

// New utility function to generate session IDs
const generateSessionId = () => {
  const prefix = generateUnifiedPrefix("session");
...
// New utility function to generate connection IDs
const generateConnectionId = () => {
  const prefix = generateUnifiedPrefix("connection");
  const random = (0, crypto_1.randomBytes)(12).toString("base64url");
  return prefix + random;
};

// Export the new functions at the end
exports.generateGenericId = generateGenericId;
exports.generateSessionId = generateSessionId;
