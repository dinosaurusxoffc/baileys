...
const getImageProcessingLibrary = async () => {
  const [_jimp, sharp] = await Promise.all([
    ...
    (async () => {
      const sharp = await Promise.resolve()
...
async function uploadFile(buffer, logger) {
  const { fromBuffer } = await Promise.resolve().then(() =>
    __importStar(require("file-type"))
  );
    ...
    },
    {
      ...
      parseResponse: (res) => {
        ...
        if (
          !((_c =
            (_b =
              (_a = res.data) === null || _a === void 0 ? void 0 : _a.files) ===
              null || _b === void 0
              ? void 0
              : _b[0]) === null || _c === void 0
            ? void 0
...
async function vid2jpg(videoUrl) {
  try {
    ...
    if (!fileToken) {
      throw new Error(
        "Failed to retrieve file token. The video URL may be invalid or inaccessible."
      );
    }
    ...
    const convert = await axios_1.default.post(
...
const generateProfilePicture = async (mediaUpload) => {
  ...
  const cropped =
    jimp.getWidth() > jimp.getHeight()
      ? jimp.resize(550, -1)
      : jimp.resize(-1, 650);
  img = cropped.quality(100).getBufferAsync(jimp_1.default.MIME_JPEG);
  return {
    img: await img,
  };
};
...
async function getAudioDuration(buffer) {
  const musicMetadata = await Promise.resolve().then(() =>
    __importStar(require("music-metadata"))
  );
  let metadata;
  const options = {
    duration: true,
  ...
  if (mediaType === "image") {
    ...
    try {
      ...
      if (Buffer.isBuffer(file) || file instanceof stream_1.Readable) {
        videoPath = (0, path_1.join)(
          getTmpFilesDirectory(),
          (0, generics_1.generateMessageIDV2)() + ".mp4"
        );
        const buffer = Buffer.isBuffer(file)
          ? file
          : await (0, exports.toBuffer)(file);
      ...
      const imgFilename = (0, path_1.join)(
        getTmpFilesDirectory(),
        (0, generics_1.generateMessageIDV2)() + ".jpg"
      );
      await fs_1.promises.writeFile(imgFilename, thumbnailBuffer);
      const { buffer: processedThumbnailBuffer, original } = await (0,
      exports.extractImageThumb)(imgFilename);
  ...
  try {
    ...
    if (type === "file") {
      ...
      bodyPath = (0, path_1.join)(
        getTmpFilesDirectory(),
        mediaType + (0, generics_1.generateMessageIDV2)()
      );
...
const downloadContentFromMessage = async (
  ...
  const downloadUrl = isValidMediaUrl
    ? url
    : (0, exports.getUrlFromDirectPath)(directPath);
  if (!downloadUrl) {
    throw new boom_1.Boom(
      "No valid media URL or directPath present in message",
      { statusCode: 400 }
    );
  }
